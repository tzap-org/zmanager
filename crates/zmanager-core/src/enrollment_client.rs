...
impl<'a, T: TzapAuthHttpTransport> TzapEnrollmentClient<'a, T> {
    ...
    pub fn request_enrollment_challenge(
        ...
        validate_challenge_payload(session, request, signing_key, csr_der, &challenge)?;
        Ok(challenge)
    }
    ...
    pub fn submit_enrollment(
        ...
        session.require_audience(SESSION_AUDIENCE_SIGN_TZAP)?;
        let challenge_bytes = jcs::canonicalize_json_bytes(&challenge.payload)
...
pub fn enroll_device_certificate(
    ...
    let payload = client.submit_enrollment(session, &challenge, signing_key, csr_der)?;
    let chain = payload.certificate_chain_der();
    let public_metadata = validator.validate_certificate_chain(&chain)?;
...
pub(crate) fn parse_enrollment_response(
    ...
    parse_certificate_payload(required_field(object, "certificate")?)
}
...
mod tests {
    use super::{
        CERTIFICATES_PATH, ENROLL_OPERATION, ENROLLMENT_CHALLENGE_CANONICALIZATION,
