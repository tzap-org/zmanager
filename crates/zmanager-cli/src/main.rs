...
fn new_formats_command(args: &[String], mut global: GlobalOptions) -> ExitCode {
    ...
    if global.json {
        println!(
            "{{\"create\":[{{\"format\":\"zip\",\"extensions\":[\".zip\"]}},{{\"format\":\"tar.zst\",\"extensions\":[\".tar.zst\",\".tzst\"]}},{{\"format\":\"7z\",\"extensions\":[\".7z\"]}}],\"extract\":[{{\"format\":\"zip\",\"extensions\":[\".zip\",\".zipx\",\".jar\",\".war\",\".ipa\",\".apk\",\".appx\",\".xpi\"]}},{{\"format\":\"tar.zst\",\"extensions\":[\".tar.zst\",\".tzst\"]}},{{\"format\":\"7z\",\"extensions\":[\".7z\"]}},{{\"format\":\"raw-stream\",\"extensions\":[\".zst\",\".gz\",\".bz2\",\".xz\",\".lzma\",\".lz\",\".br\",\".lz4\",\".lzo\",\".Z\",\".lrz\"]}},{{\"format\":\"libarchive\",\"extensions\":[\"fallback\"]}}]}}"
        );
    } else {
        println!("Create:");
        println!("  zip       .zip");
        println!("  tar.zst   .tar.zst, .tzst");
        println!("  7z        .7z");
        println!();
        println!("Extract:");
        println!("  zip       .zip, .zipx, .jar, .war, .ipa, .apk, .appx, .xpi");
        println!("  tar.zst   .tar.zst, .tzst");
        println!("  7z        .7z");
        println!("  raw       .zst, .gz, .bz2, .xz, .lzma, .lz, .br, .lz4, .lzo, .Z, .lrz");
        println!("  libarchive fallback for supported archive formats");
    }
    ExitCode::SUCCESS
}
...
fn prompt_password_from_stdin() -> Result<String, ExitCode> {
...
fn run_zip_test_new(
    ...
    match zmanager_core::zip_backend::test_zip_with_password_filter(archive, password, |name| {
            ...
            if password.is_none() =>
        {
            ...
            run_zip_test_new(archive, Some(&password), includes, excludes, global)
        }
        Err(error) => {
...
fn parse_archive_format(raw: &str) -> Result<ArchiveFormat, String> {
    match raw {
        ...
        "tar.zst" | "tzst" | "zst" => Ok(ArchiveFormat::TarZst),
...
fn temp_archive_path(destination: &Path) -> PathBuf {
    ...
    let file_name = destination
        ...
        .unwrap_or("archive");
    destination.with_file_name(format!(".{file_name}.tmp-{}-{now}", std::process::id()))
}
...
fn default_extract_destination(archive: &str) -> PathBuf {
    ...
    let stem = name
        ...
        .or_else(|| name.strip_suffix(".tzst"))
...
fn read_optional_password_stdin(enabled: bool, label: &str) -> Result<Option<String>, ExitCode> {
    if enabled {
...
fn password_arg_or_prompt(raw: Option<&str>, prompt: &str) -> Result<Option<String>, ExitCode> {
    match raw {
...
fn prompt_password(prompt: &str) -> Result<String, ExitCode> {
    ...
    if let Err(error) = io::stderr().flush() {
        eprintln!("failed to show password prompt: {error}");
...
fn is_7z_archive(path: &str) -> bool {
    path_ends_with_ignore_ascii_case(path, ".7z")
}

fn is_rar_archive(path: &str) -> bool {
    path_ends_with_ignore_ascii_case(path, ".rar") || path_ends_with_ignore_ascii_case(path, ".cbr")
}

fn is_tar_zst_archive(path: &str) -> bool {
    path_ends_with_ignore_ascii_case(path, ".tar.zst")
        || path_ends_with_ignore_ascii_case(path, ".tzst")
}

fn is_deb_archive(path: &str) -> bool {
    path_ends_with_ignore_ascii_case(path, ".deb")
}
...
fn run_zip_test(archive: impl AsRef<std::path::Path>, password: Option<&str>) -> ExitCode {
    match zmanager_core::zip_backend::test_zip_with_password(archive.as_ref(), password) {
            ...
            if password.is_none() =>
        {
            ...
            run_zip_test(archive, Some(&password))
        }
        Err(error) => {
            eprintln!("zip test failed: {error}");
            ExitCode::FAILURE
...
fn run_7z_list(archive: impl AsRef<std::path::Path>, password: Option<&str>) -> ExitCode {
    match zmanager_core::sevenz_backend::list_7z(archive.as_ref(), password) {
        ...
        Err(zmanager_core::sevenz_backend::SevenZError::PasswordRequired) if password.is_none() => {
            ...
            run_7z_list(archive, Some(&password))
        }
        Err(error) => {
