# tzap Archive Format Specification (v0.11)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.11 / 2026-05-20.5 (draft after validation/caps/interoperability review) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-20 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.10 text, this v0.11 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.10

This revision fixes remaining validation, cap, and interoperability
ambiguities identified after v0.10.

1. **Previously undocumented fields are now normative.** `FileEntry.flags`
   is reserved and must be zero; `payload_block_count` is defined.
   (§15.2, §15.6, §29)
2. **Encrypted object sizes are unambiguous.** `encrypted_size` now means
   total ciphertext length including AEAD tag after suffix padding, and
   must equal `data_block_count * block_size` in v0.11. (§15.3, §15.4,
   §15.8, §15.9, §18)
3. **Empty archives are explicitly valid.** A zero-file archive has no
   payload envelopes and `shard_count = 0`, but still has an IndexRoot,
   ManifestFooter, and trailer. (§15.2, §19.1)
4. **Reader caps are more concrete.** Per-shard file counts and adjacent
   hash-collision shard scans now have explicit caps. (§13.3, §15.4,
   §15.9)
5. **FEC object layout is fully normative.** Object block ranges are
   contiguous, size fields must fit `u32`, and parity iteration has a
   bounded convergence rule. (§18, §27, §29)
6. **Header/trailer binding is explicit.** Readers must compare
   authenticated trailer/footer identity fields against VolumeHeader
   fields before using bootstrap data. (§17.1)
7. **Path and metadata diagnostics are tightened.** Writers must not emit
   unsafe archive paths, and library readers should return structured
   metadata-fidelity diagnostics. (§16, §29)
8. **Dictionary streaming caveat is explicit.** Dictionary-compressed
   single-stream archives without an already-available sidecar may require
   unbounded buffering; writers must not advertise live pipe
   decompression in that mode. (§7.5, §19.3)
...
- `pad_len = 0` is not valid in v0.11. The extra block in the exact-fit
...
stream), conforming v0.11 writers MUST use `stripe_width = 1`,
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.11.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.11
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.11 this
...
Completed v0.11 writers MUST set `is_authoritative = 1` in every closed
...
non-critical by a future version; for v0.11, unknown flag bits are a hard
error.
...
For every encrypted object in v0.11, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.11. Writers MUST set it to zero, and
...
payload-data block order be reconstructable.
...
be zero in v0.11 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.11 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.11 mode that round-robins
...
### 20.1 Padding overhead (v0.11 unchanged from v0.10)
...
Same as v0.10. Envelope-level AEAD, object-local FEC encoding, zstd frame
...
implementation claims conformance to this v0.11 draft, incompatible
...
## Appendix A: All changes from v0.10 → v0.11

| Section | Change |
|---|---|
| §6.1 / §20.1 | Padding marker rationale clarified; exact-fit worst-case overhead documented |
| §13.3 / §15.4 / §15.9 | Added `max_files_per_index_shard` and `max_hash_collision_shard_scan` reader caps |
| §15.2 / §19.1 | Empty archives explicitly allowed |
| §15.2 | `payload_block_count` defined as total payload-data BlockRecords |
| §15.3 / §15.4 / §15.8 / §15.9 / §18 / §29 | `encrypted_size` canonicalized as `data_block_count * block_size`, with u32 overflow rejection |
| §15.6 / §29 | `FileEntry.flags` reserved; writers set zero and readers reject non-zero |
| §16 / §29 | Writers forbidden from emitting unsafe paths; structured metadata diagnostics recommended for libraries |
| §17.1 | Authenticated trailer/footer identity fields must match VolumeHeader before bootstrap data is used |
| §17.3 | Multi-volume sequential reconstruction note added |
| §18 | FEC object block ranges made explicitly contiguous |
| §19.3 | Dictionary-compressed live pipe mode cannot be advertised without a ready bootstrap sidecar |
| §27 | Parity recurrence capped at 100 iterations |
| §28.1 | v0.11 test corpus additions cover empty archives, FileEntry flags, encrypted-size canonicality, caps, header/trailer binding, unsafe paths, and parity convergence |

---

*End of v0.11 specification.*
