# tzap Archive Format Specification (v0.9)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.9 / 2026-05-20.2 (draft after interoperability/clarity review) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-20 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.8 text, this v0.9 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.8

This revision fixes the remaining interoperability and misleading-wording
issues identified after v0.8.

1. **HKDF is now fully specified.** Subkeys use HKDF-SHA-256
   Extract-then-Expand with a fixed domain salt, explicit info labels,
   and 32-byte outputs. (§13.2)
2. **IndexRoot nonce and AAD counters are aligned.** The singleton
   IndexRoot uses counter 0 for both nonce derivation and AAD. (§14.3)
3. **`chunk_size` is no longer dangling.** It is defined as the writer's
   target uncompressed frame chunk size for large tar member groups, not
   as a reader parsing boundary. (§6.2, §9, §24)
4. **ManifestFooter bootstrap dependency is explicit.** IndexRoot FEC is
   object-local only after a reader has obtained an authenticated
   ManifestFooter or bootstrap sidecar that locates it. (§11, §18)
5. **The write algorithm no longer implies on-disk backfill.**
   FrameEntry envelope references are recorded in memory before index
   serialization. (§19.1)
6. **Bit-rot parity wording is clarified.** The bit-rot buffer is
   additional conservative margin after volume-loss parity sizing. (§27)
...
- `pad_len = 0` is not valid in v0.9. The extra block in the exact-fit
...
stream), conforming v0.9 writers MUST use `stripe_width = 1`,
`volume_loss_tolerance = 0`, and either `has_dictionary = 0` or a
bootstrap sidecar containing an authenticated encrypted IndexRoot copy
(§12.2, §17.3). A live reader cannot decompress dictionary-compressed
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.9.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bit 1: reserved; MUST be zero in v0.9
    _reserved:     [u8; 2],
...
Completed v0.9 writers MUST set `is_authoritative = 1` in every closed
...
uses the sidecar's authenticated encrypted IndexRoot copy to recover the
dictionary before payload decompression. If a reader starts from a live
non-seekable stream before the sidecar is complete, it MUST either buffer
encrypted payload bytes until the dictionary is recovered or reject with
...
comes from the ManifestFooter HMAC and IndexRoot AEAD verification.
...
non-critical by a future version; for v0.9, unknown flag bits are a hard
error.
...
struct IndexRoot {
    ...
    // Directory-prefix locality table (§15.8; required for large archives)
...
DirectoryHintEntry records are sorted by `(dir_hash, directory_path)`
using bytewise comparison of normalized UTF-8 directory paths as the
collision tie-breaker. If multiple directory paths share the same
`dir_hash`, readers MUST compare the actual string from the hint string
pool. `shard_list_offset` points into a contiguous u32 shard-ID array;
the `shard_count` IDs for an entry MUST be sorted ascending and unique.
...
the candidate shard(s). The Directory Hint Table is not a full
...
- `EnvelopeEntry.first_frame_index + frame_count` is within the
  FrameEntry table;
...
- `FileEntry.first_frame_index + frame_count` is within the FrameEntry
  table;
...
provides an authenticated encrypted IndexRoot copy from which those bytes
can be recovered, the reader MUST reject with "dictionary bootstrap
required." If the payload stream is already flowing before the sidecar is
available, the reader MUST buffer encrypted envelope bytes until the
dictionary is recovered or reject; it MUST NOT attempt dictionary-less
decompression.
...
No part of the v0.9 write path needs to revisit a closed S3 part or to
...
containing an authenticated encrypted IndexRoot copy (§12.2), otherwise
non-seekable sequential extraction would be impossible.
...
writing final volumes. There is no conforming v0.9 mode that round-robins
...
### 20.1 Padding overhead (v0.9 unchanged from v0.8)
...
Same as v0.8. Envelope-level AEAD, object-local FEC encoding, zstd frame
...
implementation claims conformance to this v0.9 draft, incompatible
...
| `index_fec_data_shards` | 16 | maximum index-shard data blocks |
| `index_fec_parity_shards` | derived from V and N | maximum index-shard parity blocks; actual count is per-object (§27) |
| `index_root_fec_data_shards` | 4 | Increase if dictionary/hints make IndexRoot larger |
| `index_root_fec_parity_shards` | derived from V and N | maximum IndexRoot parity blocks; actual count is per-object (§27) |
...
Crate selection unchanged from v0.8. Module layout unchanged.

### 28.1 Test corpus additions for v0.9
...
- **Bootstrap sidecar**: dictionary archive with a bootstrap sidecar;
  ...
  and IndexRoot AEAD are checked; verify IndexRoot decrypts from the
  sidecar BlockRecord copy and the dictionary is available before
  payload frame decompression.
...
16. Validates §12.3 sidecar CRC, UUID/session binding, known flag bits,
    ManifestFooter HMAC, and IndexRoot AEAD before trusting bootstrap
    sidecar bytes.
...
## Appendix A: All changes from v0.8 → v0.9

| Section | Change |
|---|---|
| §6.2 / §9 / §24 | `chunk_size` defined as writer target uncompressed zstd-frame chunk size, not a reader parsing boundary |
| §11 / §18 | ManifestFooter or bootstrap sidecar dependency for locating and repairing IndexRoot clarified |
| §13.2 | HKDF-SHA-256 Extract/Expand schedule fully specified with salt, info labels, and output lengths |
| §14.1 | `hkdf_expand_sha256` in nonce derivation defined as HKDF-Expand-SHA-256 |
| §14.3 | IndexRoot uses counter 0 consistently in nonce derivation and AAD |
| §19.1 | FrameEntry envelope references are recorded in memory before index serialization; no on-disk backfill implied |
| §27 | Bit-rot parity buffer described as conservative additional margin after volume-loss sizing |
| §28.1 | v0.9 test corpus additions cover HKDF vectors, IndexRoot counter binding, `chunk_size`, and bootstrap dependency |

---

*End of v0.9 specification.*
