# tzap Archive Format Specification (v0.10)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.10 / 2026-05-20.4 (draft after constants/ranges/sequential/scalability review) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-20 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.9 text, this v0.10 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.9

This revision fixes remaining constants, byte-range notation, sequential
read, and minor clarity issues identified after v0.9.

1. **AEAD constants are defined.** Each AEAD algorithm now declares
   nonce and tag lengths; `AEAD_NONCE_LEN` and `AEAD_TAG_LEN` are derived
   from the selected algorithm. (§5, §14)
2. **Integrity ranges use explicit byte counts.** CRC and HMAC comments
   now say "first N bytes" / "all bytes before this field" instead of
   ambiguous range notation. (§8, §11, §12)
3. **Sequential envelope counters are normative.** Payload envelopes are
   assigned contiguous `envelope_index` values starting at 0, and
   sequential readers maintain that counter while skipping parity blocks.
   (§15.3, §17.3, §19.1)
4. **Reserved BlockRecord flags are enforced by readers.** Writers keep
   bit 1 zero; readers reject blocks with reserved flag bits set. (§10,
   §18, §29)
5. **Small clarity fixes.** `IndexRoot._padding` is renamed to
   `_reserved2`, tar/frame wording is tightened, DHT conformance names
   the threshold directly, and FEC parity iteration starts at 0.
   (§6.2, §15.2, §15.8, §27, §29)
6. **IndexRoot is no longer a monolith.** Global EnvelopeEntry and
   FrameEntry tables are removed from IndexRoot and stored with the
   IndexShard(s) whose FileEntry records reference them. Random access no
   longer requires downloading a multi-GB root object. (§15.2-§15.7)
7. **Large metadata objects respect GF(2^16) limits.** Dictionary bytes
   and Directory Hint data are separate encrypted/FEC objects, and
   Reed-Solomon GF(2^16) object shard limits are explicit. (§12.3, §15.8,
   §18)
8. **Directory hints use 64-bit internal offsets and can be sharded.**
   Huge directory maps no longer depend on 32-bit table offsets. (§15.8)
...
- `pad_len = 0` is not valid in v0.10. The extra block in the exact-fit
...
stream), conforming v0.10 writers MUST use `stripe_width = 1`,
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.10.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
| `0x0005` | UTF-8 | Locale tag for filenames |
| ~~`0x0006`~~ | ~~bytes~~ | **Removed; moved to encrypted metadata.** A writer setting `has_dictionary = 1` declares that IndexRoot contains a dictionary-object extent (§15.2). |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.10
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.10 this
...
Completed v0.10 writers MUST set `is_authoritative = 1` in every closed
...
non-critical by a future version; for v0.10, unknown flag bits are a hard
error.
...
reader resource caps are reached. If caps are reached before the run
ends, the reader MUST fail with "hash-prefix collision run exceeds
resource caps" rather than returning a partial lookup result.
...
shards are the scaling mechanism.
...
be zero in v0.10 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.10 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.10 mode that round-robins
...
### 20.1 Padding overhead (v0.10 unchanged from v0.9)
...
Same as v0.9. Envelope-level AEAD, object-local FEC encoding, zstd frame
...
implementation claims conformance to this v0.10 draft, incompatible
...
| `index_root_fec_data_shards` | dynamic, minimum 16 | maximum IndexRoot and dictionary-object data blocks; writer MUST raise enough for serialized IndexRoot |
...
## Appendix A: All changes from v0.9 → v0.10

| Section | Change |
|---|---|
| §5 / §14 | AEAD nonce and tag lengths defined for every registered AEAD algorithm |
| §6.2 / §19.1 | Tar/frame wording clarified and `chunk_size` use made explicit in frame splitting |
| §8 / §11 / §12 | CRC/HMAC coverage changed to explicit byte counts and "before this field" wording |
| §10 / §18 / §29 | Reserved BlockRecord flag bits must be zero and readers must reject if set |
| §12.3 / §15.2 / §19.3 | Dictionary bytes moved out of IndexRoot into a separate encrypted dictionary object; bootstrap sidecar can carry dictionary BlockRecords |
| §15.1-§15.7 / §17.2 / §19.1 | Global FrameEntry and EnvelopeEntry tables moved from IndexRoot into shard-local IndexShard tables |
| §15.1 / §15.4 / §15.7 / §29 | File sorting tie-breaker made explicit: `(SHA-256(path)[0..8], normalized path bytes)` |
| §15.2 | `IndexRoot._padding` renamed to `_reserved2`; IndexRoot is bounded root metadata rather than a monolithic index |
| §15.3 / §17.3 / §19.1 | Payload envelope indices are contiguous from 0; sequential extraction derives AEAD counters from this order |
| §15.8 / §29 | Directory hints use 64-bit internal offsets, may be split into encrypted hint shards, and conformance states the file-count threshold directly |
| §18 / §24 / §29 | ReedSolomonGF16 per-object 65,535-shard limit made explicit; IndexRoot FEC sizing made dynamic |
| §27 | FEC parity recurrence initialization set to `G_parity = 0` |
| §28.1 | v0.10 test corpus additions cover AEAD constants, checksum ranges, sequential counters, reserved flags, parity initialization, large-index bounds, directory hint sharding, and GF16 limits |

---

*End of v0.10 specification.*
