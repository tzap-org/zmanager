# tzap Archive Format Specification (v0.8)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.8 / 2026-05-20.1 (draft after stream/FEC consistency review) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-20 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.7 text, this v0.8 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.7

This revision fixes stream/bootstrap, FEC sizing, and remaining
confidentiality contradictions identified after v0.7.

1. **Dictionary and non-seekable streaming are no longer contradictory.**
   No-sidecar sequential extraction requires `has_dictionary = 0`.
   Dictionary archives can be streamed only when a trusted bootstrap
   sidecar is available before the reader decompresses payload frames.
   (§12.2, §12.3, §17.3, §19.3.)
2. **The unencrypted tar-stream hash extension is removed.** CryptoHeader
   tag `0x0004` is forbidden in v0.8; content hashes remain encrypted in
   IndexRoot. (§9.1, §22.)
3. **Object-local FEC parity is derived per object.** Table entries now
   record each object's actual parity count. The global default no longer
   implies 48 parity blocks on every 1 MiB envelope. (§18, §27.)
4. **Volume-loss tolerance is constrained.** Writers must enforce
   `0 ≤ N < V`; fully non-reopenable `V = 1` streaming forces
   `N = 0`. (§7.3, §19.3, §27.)
5. **Directory hints are specified enough to implement.** Normalization,
   sorting, collision handling, and lookup behavior are now defined, and
   the text clarifies that hints accelerate prefix selection but are not
   a complete alphabetical listing index. (§15.8.)
6. **Sidecar wording no longer overpromises random access.** A sidecar can
   provide bootstrap metadata, but non-seekable payload streams still need
   buffering or range-capable storage for random reads. Its header and
   IndexRoot block copy are now specified. (§12.2, §12.3, §17.1.)
7. **Cross-table validation is explicit.** Frame, envelope, and file
   extents must be range-checked against each other before decoding.
   (§15.9.)
...
- `pad_len = 0` is not valid in v0.8. The extra block in the exact-fit
...
stream), conforming v0.8 writers MUST use `stripe_width = 1`,
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.8.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bit 1: reserved; MUST be zero in v0.8
    _reserved:     [u8; 2],
...
Completed v0.8 writers MUST set `is_authoritative = 1` in every closed
...
non-critical by a future version; for v0.8, unknown flag bits are a hard
error.
...
No part of the v0.8 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.8 mode that round-robins
...
### 20.1 Padding overhead (v0.8 unchanged from v0.7)
...
Same as v0.7. Envelope-level AEAD, object-local FEC encoding, zstd frame
...
Crate selection unchanged from v0.7. Module layout unchanged.

### 28.1 Test corpus additions for v0.8
...
## Appendix A: All changes from v0.7 → v0.8

| Section | Change |
|---|---|
| §7.3 / §19.3 / §27 | Enforced `0 ≤ N < V`; `V = 1` streaming must use `N = 0` |
| §9.1 | CryptoHeader extension `0x0004` forbidden; content hash remains encrypted in IndexRoot |
| §12.2 / §12.3 / §17.3 | Bootstrap sidecar layout defined; dictionary archives require sidecar/dictionary bootstrap for non-seekable sequential reads |
| §14.3 | Index encryption pseudocode now explicitly pads before AEAD |
| §15.8 | Directory Hint Table normalization, sorting, collision handling, and listing limitations specified |
| §15.9 | Cross-table validation rules added for FrameEntry, EnvelopeEntry, and FileEntry extents |
| §18 / §27 | FEC class maxima separated from actual per-object parity counts |
| §18 | Synthetic zero FEC shards made virtual; BlockRecord reserved flag bit clarified |
| §28.1 | v0.8 test corpus added for dictionary streaming, per-object FEC, forbidden extension, and V/N constraints |
| §29 | Conformance requirements updated |

---

*End of v0.8 specification.*
