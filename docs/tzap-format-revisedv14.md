# tzap Archive Format Specification (v0.14)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.14 / 2026-05-21.1 (draft after shard-version, HMAC, and sidecar review) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.13 text, this v0.14 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.13

This revision fixes shard extensibility, HMAC domain separation,
sidecar packing, KDF caps, and several index consistency ambiguities
identified after v0.13.

1. **Draft visibility is now on the wire.** `volume_format_rev = 14` for
   this draft, and readers reject newer revisions they do not implement.
   (§8, §17.1, §23)
2. **IndexShard has an in-band version.** `IndexShardHeader` now carries
   `version = 1` and reserved bytes; readers reject unknown shard
   versions. (§15.5, §15.9, §23)
3. **All HMACs use domain-separated inputs.** CryptoHeader,
   ManifestFooter, VolumeTrailer, and BootstrapSidecarHeader now use the
   same `domain || archive_uuid || session_id || bytes` template. (§9,
   §11, §12, §12.3, §28.1)
4. **Sidecars are packed and fully bounded.** v0.14 sidecars no longer
   allow unauthenticated padding/extension gaps between authenticated
   sections. (§12.3)
5. **Index consistency rules are stricter.** Duplicate shard-local
   FrameEntry/EnvelopeEntry rows must be byte-identical when observed,
   shard intervals must be sorted/non-overlapping, and volume-count
   fields must agree. (§15.9, §17.1)
6. **Dictionary and empty-archive invariants are explicit.** Dictionary
   extents must be zero whenever `has_dictionary = 0`; empty archives use
   `SHA-256(b"")` and `tar_total_size = 0`. (§15.2)
7. **KDF and sizing caps are enforceable.** Raw KDF params are defined,
   Argon2id salt length is capped, KDF caps are mandatory, block size and
   envelope target minima are explicit, and parity arithmetic is checked.
   (§9, §13, §17.1, §24, §27)
8. **Directory hints now have a lookup procedure.** Directory path
   normalization, shard-list alignment, and hint lookup are normative.
   (§15.8, §15.9)
...
- `pad_len = 0` is not valid in v0.14. The extra block in the exact-fit
...
stream), conforming v0.14 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 14 for this draft
...
`chunk_size = 0`, `envelope_target_size = 0`, or `block_size < 4096`.
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.14.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.14
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.14 this
...
Within a single volume, BlockRecords MUST appear in strictly increasing
`block_index` order. For a complete volume set, no two BlockRecords may
share the same `block_index`, and the observed global block indices MUST
cover every value from 0 through the final emitted block index. Object
extents declared by ManifestFooter, ShardEntry, DirectoryHintShardEntry,
or EnvelopeEntry MUST refer to contiguous global block-index ranges.
Readers MUST reject duplicates, decreasing order, gaps in a complete
input set, or any missing block required by an operation when no recovery
mode can repair it.
...
Completed v0.14 writers MUST set `is_authoritative = 1` in every closed
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.14
bootstrap sidecar. If the ManifestFooter is present, its offset MUST be
128. If IndexRoot records are present, their offset MUST immediately
follow the ManifestFooter when present, otherwise 128. If dictionary
records are present, their offset MUST immediately follow the IndexRoot
records. The sidecar file size MUST equal the end of the last declared
range.
...
non-critical by a future version; for v0.14, unknown flag bits are a hard
error.
...
`Argon2idParams` followed by `salt_length` raw salt bytes. Writers MUST
use `8 ≤ salt_length ≤ 64`; readers MUST reject salts outside that
range or KDF parameter buffers that do not fit inside CryptoHeader.
...
`RawKdfParams`; user supplies the 32-byte `master_key` via keyfile. No
KDF salt is stored for raw mode because HKDF-Extract already uses the
archive UUID and session ID as public per-archive/session salt (§13.2).
Readers MUST reject a KdfParams `algo_tag` that does not match
`CryptoHeader.kdf_algo`.
...
For every encrypted object in v0.14, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.14. Writers MUST set it to zero, and
...
- when the same `frame_index` or `envelope_index` appears in more than
  one loaded IndexShard local table, every duplicated FrameEntry or
  EnvelopeEntry row MUST be byte-identical. Readers MUST reject on
  mismatch. A full-archive `verify` operation MUST check this globally
  across all IndexShards.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   only v0.14 conformance rejects `volume_format_rev != 14`.
...
3. Parse KdfParams; prompt for passphrase or load keyfile. If
   ...
   `salt_length` is outside 8..64 bytes.
4. Run KDF → master_key. Derive mac_key using the archive UUID and
   ...
   Verify `VolumeHeader.stripe_width == CryptoHeader.stripe_width`;
   reject on mismatch.
...
6. If the input is seekable:
     ...
     d. Read ManifestFooter at trailer's manifest_footer_offset / length.
        ...
        VolumeHeader.stripe_width`; reject on mismatch.
...
be zero in v0.14 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.14 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.14 mode that round-robins
...
### 20.1 Padding overhead (v0.14 unchanged from v0.13)
...
Same as v0.13. Envelope-level AEAD, object-local FEC encoding, zstd frame
...
document uses `format_version = 1` and `volume_format_rev = 14`. Readers
...
`volume_format_rev = 14`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.14 draft, incompatible
...
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 14; verify v0.14-only readers accept only
  14 and reject newer revisions.
...
- **KDF parameter caps**: Argon2id salt lengths below 8 and above 64,
  `t_cost`, `m_cost_kib`, and `parallelism` over reader caps, and raw
  mode with `RawKdfParams { algo_tag = 0 }`. Verify all invalid cases
  reject before expensive allocation or KDF execution.
...
- **Bootstrap sidecar**: dictionary archive with a bootstrap sidecar;
  ...
  frame decompression. Verify sidecars with padding, extension bytes,
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1` and `volume_format_rev = 14`.
...
32. Writes BlockRecords for each volume in strictly increasing
    `block_index` order and never emits duplicate global block indices.
...
34. Sets all IndexRoot dictionary fields to zero whenever
    `has_dictionary = 0`.
35. Emits v0.14 bootstrap sidecars as packed sequences with no padding,
...
27. Verifies that BlockRecords within a volume are strictly increasing by
    `block_index`, and when reconstructing a complete global order across
    volumes, verifies that no two BlockRecords share the same
    `block_index` and that no global block index is missing. Duplicate or
    decreasing block indices are hard errors; gaps inside a declared
    object extent are hard errors unless the reader is in an explicit
    recovery mode that can repair the missing blocks.
28. Rejects unsupported `volume_format_rev`, mismatched
    `VolumeHeader.stripe_width`, `CryptoHeader.stripe_width`, or
    `ManifestFooter.total_volumes`, and excessive KDF parameters before
    attempting expensive work.
29. Rejects `has_dictionary = 0` with non-zero IndexRoot dictionary
    fields, unrecognized IndexShard versions, and non-identical duplicate
    FrameEntry/EnvelopeEntry rows when observed.
...
## Appendix A: All changes from v0.13 → v0.14

| Section | Change |
|---|---|
| §8 / §17.1 / §23 | `volume_format_rev = 14` added as draft-level wire revision guard |
| §9 / §11 / §12 / §12.3 | HMAC inputs use consistent domain-separated UUID/session binding |
| §12.3 | Bootstrap sidecars are packed; padding/extension gaps and trailing bytes are forbidden |
| §13 / §17.1 | RawKdfParams defined, Argon2id salt length capped, KDF caps enforced |
| §15.2 | Empty archive hash defined as `SHA-256(b"")`; dictionary fields zero whenever `has_dictionary = 0` |
| §15.5 / §15.9 / §23 | IndexShardHeader gains `version = 1` and reserved bytes |
| §15.8 / §15.9 | Directory hint lookup algorithm and shard-list base alignment made normative |
| §15.9 / §29 | Duplicate FrameEntry/EnvelopeEntry rows must agree; shard interval and block-index ordering checks added |
| §17.1 / §29 | VolumeHeader, CryptoHeader, and ManifestFooter volume-count fields must agree |
| §24 / §27 | Block/envelope minima, checked parity arithmetic, bit-rot cap, and u16 parity ceiling documented |
| §28.1 / §29 | v0.14 tests and conformance checklist expanded |

---

*End of v0.14 specification.*
