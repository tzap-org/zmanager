# tzap Archive Format Specification (v0.17)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.17 / 2026-05-21.1 (draft after sidecar and lookup clarification review) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.16 text, this v0.17 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.16

This revision clarifies bootstrap-sidecar semantics and directory-hint
lookup, and adds a few low-cost cross-checks found during implementation
readiness review.

1. **Draft visibility is bumped.** `volume_format_rev = 17` for this
   draft. (§8, §17.1, §23)
2. **Sidecar ManifestFooter volume matching is clarified.** A
   ManifestFooter copied into a bootstrap sidecar uses `volume_index = 0`;
   readers verify UUID/session/total volume counts but do not compare
   that sidecar footer to the currently opened volume index. (§12.2,
   §12.3, §17.1)
3. **Directory-hint shard IDs are resolved through ShardEntry.** The
   lookup algorithm now explicitly maps each hinted shard ID back to the
   IndexRoot ShardEntry table before reading an IndexShard. (§15.8)
4. **ManifestFooter size is a normative constant.** `sizeof(ManifestFooter)
   = 136` bytes. (§11, §12.3, §17.1)
5. **Envelope frame totals are cross-checked.** The sum of compressed
   frame sizes for each payload envelope must equal
   `EnvelopeEntry.plaintext_size`. (§15.9)
6. **VolumeTrailer bytes_written is checked.** Seekable readers compare
   authenticated `bytes_written` with `file_size - 128`. (§12, §17.1)
7. **CryptoHeader offset flexibility is explicitly bounded.** v0.17 keeps
   variable `crypto_header_offset`, but reiterates that VolumeHeader
   identity and pointers are unauthenticated until HMAC checks succeed.
   (§8, §9, §17.1)
8. **Writer chunk sizing is stricter.** Writers now MUST set
   `chunk_size <= envelope_target_size`; readers still treat it as
   advisory metadata. (§9, §29)
...
- `pad_len = 0` is not valid in v0.17. The extra block in the exact-fit
...
stream), conforming v0.17 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 17 for this draft
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.17.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.17
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.17 this
...
Completed v0.17 writers MUST set `is_authoritative = 1` in every closed
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.17
...
non-critical by a future version; for v0.17, unknown flag bits are a hard
error.
...
For every encrypted object in v0.17, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.17. Writers MUST set it to zero, and
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.17.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   only v0.17 conformance rejects `volume_format_rev != 17`.
...
be zero in v0.17 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.17 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.17 mode that round-robins
...
### 20.1 Padding overhead (v0.17 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 17`. Readers
...
`volume_format_rev = 17`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.17 draft, incompatible
...
### 28.1 Test corpus additions for v0.17
...
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 17; verify v0.17-only readers accept only
  17 and reject newer revisions.
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1` and `volume_format_rev = 17`.
...
20. Sets `chunk_size` to a non-zero writer target no larger than
...
34. Sets all IndexRoot dictionary fields to zero whenever
    `has_dictionary = 0`.
35. Emits v0.17 bootstrap sidecars as packed sequences with no padding,
...
## Appendix A: All changes from v0.16 → v0.17

| Section | Change |
|---|---|
| §8 / §9 / §17.1 | CryptoHeader offset flexibility clarified as bounded unauthenticated input |
| §11 / §12.3 / §17.1 | `sizeof(ManifestFooter) = 136` made normative |
| §12 / §17.1 | `VolumeTrailer.bytes_written` checked against seekable file size |
| §12.2 / §12.3 / §17.1 / §29 | Bootstrap sidecar ManifestFooter `volume_index = 0`; sidecar bootstrap skips per-volume index comparison |
| §15.8 | Directory-hint shard IDs explicitly resolve through IndexRoot ShardEntry records |
| §15.9 | Envelope frame compressed-size coverage must equal `EnvelopeEntry.plaintext_size` |
| §9 / §29 | Writers must set `chunk_size <= envelope_target_size` |
| §28.1 / §29 | v0.17 tests and conformance checklist expanded |

---

*End of v0.17 specification.*
