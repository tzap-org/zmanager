# tzap Archive Format Specification (v0.6)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.6 (draft after architecture hardening review) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5 |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.5

This revision fixes the remaining architecture issues identified after
v0.5. These are larger than boundary checks: they change the normative
random-access model, FEC metadata model, and confidentiality claims.

1. **Random access now uses frame extents, not tar offsets inside
   encrypted envelopes.** FileEntry no longer pretends that a byte offset
   inside envelope plaintext points at a tar header. It points to an
   ordered range of zstd frames and an offset in the decompressed bytes
   of the first frame. (§15.3, §15.6, §15.7.)
2. **FEC is now object-local and discoverable.** Each encrypted object
   (payload envelope, index shard, IndexRoot) stores enough data/parity
   block counts to repair it without guessing global FEC group layout.
   (§11, §15.3, §15.4, §18.)
3. **ManifestFooter no longer exposes content hashes or tar size.**
   Sensitive archive totals move into encrypted IndexRoot. The security
   section now states the remaining traffic-analysis leakage explicitly.
   (§11, §15.2, §22.)
4. **Single-sink streaming is made honest.** Fully non-reopenable
   single-sink streaming is supported for V=1. Striped multi-volume
   archives require concurrent or append-reopenable sinks, or local
   spooling before upload/write. (§7.4, §7.5, §19.3.)
5. **Tar metadata claims are narrowed and made profile-based.** POSIX
   ustar metadata is baseline; PAX/GNU extensions are required for
   xattrs, ACLs, sparse files, and nanosecond timestamps. (§16.)
6. **HMAC coverage is byte-exact.** CryptoHeader, ManifestFooter, and
   VolumeTrailer now define exact HMAC byte ranges. (§9, §11, §12.)
7. **Parity example corrected.** The default auto-scaling example now
   converges to 48 parity shards, not approximately 45. (§27.)
...
stream), conforming v0.6 writers MUST use `stripe_width = 1`.
...
Completed v0.6 writers MUST set `is_authoritative = 1` in every closed
...
11. Build index (compute SHA-256(path) for every FileEntry, sort by hash):
     a. Partition into shards of ~10,000 files each (default)
        without splitting identical 8-byte hash-prefix runs.
     ...
     c. Build Index Root: encrypted archive totals + shard table +
        ...
        (if has_dictionary = 1) + optional Directory Hint Table.
...
No part of the v0.6 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.6 mode that round-robins
...
### 20.1 Padding overhead (v0.6 unchanged from v0.5)
...
Same as v0.5. Envelope-level AEAD, object-local FEC encoding, zstd frame
...
Crate selection unchanged from v0.5. Module layout unchanged.

### 28.1 Test corpus additions for v0.6
...
13. Emits PAX/GNU tar extension records when claiming metadata beyond
   ustar baseline.
14. Auto-scales `G_parity` per §27 unless `--unsafe-parity` is set.
...
## Appendix A: All changes from v0.5 → v0.6

| Section | Change |
|---|---|
| §1 / §6 | Random access redefined around tar member groups and zstd frame extents |
| §7.4 / §7.5 | Striped multi-volume writing now requires concurrent, append-reopenable, or spooled sinks; fully non-reopenable streaming is V=1 |
| §9 | CryptoHeader HMAC coverage made byte-exact |
| §11 | ManifestFooter reduced to bootstrap metadata; content hash and tar totals move into encrypted IndexRoot |
| §15.2 | IndexRoot now stores encrypted archive totals and content SHA-256 |
| §15.3 | EnvelopeEntry records object-local data/parity counts and frame range |
| §15.3 | FrameEntry clarified as compressed-frame offsets inside envelope plaintext |
| §15.4 | ShardEntry records object-local data/parity counts |
| §15.6 | FileEntry now addresses decompressed frame extents, not envelope offsets |
| §15.7 / §17.2 | Random extraction algorithm rewritten to decode frame ranges and slice tar member group bytes |
| §16 | Metadata preservation narrowed to explicit tar extension profiles |
| §18 | FEC changed from underspecified global groups to object-local repairable objects |
| §19.1 | Write algorithm rewritten around tar member groups, frame packing, and object-local FEC |
| §22 | Security analysis now states remaining traffic-analysis leakage explicitly |
| §27 | Parity auto-scaling example corrected to `G_parity = 48` |
| §29 | Conformance requirements updated |

---

*End of v0.6 specification.*
