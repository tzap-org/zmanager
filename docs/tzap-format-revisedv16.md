# tzap Archive Format Specification (v0.16)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.16 / 2026-05-21.1 (draft after depadding, frame, and shard validation review) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.15 text, this v0.16 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.15

This revision tightens validation between depadded envelope plaintext,
frame metadata, shard identity, and full-archive totals. It also clarifies
several reader rules where v0.15 relied on cross-references.

1. **Draft visibility is bumped.** `volume_format_rev = 16` for this
   draft. (§8, §17.1, §23)
2. **Depadded payload length is now bound to EnvelopeEntry.** After suffix
   padding is stripped, the resulting payload length MUST equal
   `EnvelopeEntry.plaintext_size` before any frame slice is taken.
   (§6.1, §15.9, §17.2)
3. **File and frame entries have non-empty minima.** FileEntries require
   `frame_count ≥ 1` and `tar_member_group_size ≥ 512`; FrameEntries
   require non-zero compressed and decompressed sizes. (§15.3, §15.6,
   §15.9)
4. **Full verify checks tar-stream totals.** `IndexRoot.tar_total_size`
   must match the highest observed frame end offset across distinct
   frames. (§15.9)
5. **IndexShard identity is cross-checked.** A decrypted IndexShard's
   `shard_index` must match the ShardEntry used to locate it, and
   non-empty archives cannot contain empty IndexShards. (§15.5, §15.9,
   §17.2)
6. **Directory hint shard-list bounds are explicit.** Shard-list entries
   are now addressed by u32 index rather than byte offset, and the final
   array range is checked exactly. (§15.8, §15.9)
7. **BlockRecord end flags are clearer.** Bit 0 is set on the last data
   block of every encrypted object kind, not just payload envelopes.
   (§10)
8. **Parity ceiling formulas are explicit.** The integer forms for both
   ceiling divisions are normative. (§27)
...
- `pad_len = 0` is not valid in v0.16. The extra block in the exact-fit
...
stream), conforming v0.16 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 16 for this draft
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.16.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.16
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.16 this
...
Completed v0.16 writers MUST set `is_authoritative = 1` in every closed
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.16
...
non-critical by a future version; for v0.16, unknown flag bits are a hard
error.
...
For every encrypted object in v0.16, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.16. Writers MUST set it to zero, and
...
- an IndexShardHeader's `shard_index` matches the ShardEntry used to
  locate that shard object;
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.16.
...
- in full-archive `verify`, `IndexRoot.tar_total_size` MUST equal the
  highest observed `FrameEntry.tar_stream_offset +
  FrameEntry.decompressed_size` across all distinct payload frames. Empty
  archives have no payload frames and MUST use `tar_total_size = 0`.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   only v0.16 conformance rejects `volume_format_rev != 16`.
...
be zero in v0.16 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.16 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.16 mode that round-robins
...
### 20.1 Padding overhead (v0.16 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 16`. Readers
...
`volume_format_rev = 16`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.16 draft, incompatible
...
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 16; verify v0.16-only readers accept only
  16 and reject newer revisions.
...
- **Directory hint sharding**: build directory hints whose entry table
  ...
  `start_index + shard_count` exceeds the array length.
...
- **Structural validation**: malformed IndexRoot and IndexShard buffers
  ...
  before allocation. Include an IndexShard whose
  `IndexShardHeader.shard_index` does not match the locating ShardEntry
  and an IndexShard with `file_count = 0`.
...
- **Archive totals**: mutate `IndexRoot.payload_block_count` so it no
  ...
  `data_block_count` values, and mutate `IndexRoot.tar_total_size` so it
  no longer equals the highest observed frame end offset; verify
  full-archive `verify` rejects.
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1` and `volume_format_rev = 16`.
...
25. Sets `FileEntry.flags = 0` and emits no unsafe archive paths
    ...
    greater than `CryptoHeader.max_path_length`; every FileEntry has
    `frame_count ≥ 1` and `tar_member_group_size ≥ 512`.
...
34. Sets all IndexRoot dictionary fields to zero whenever
    `has_dictionary = 0`.
35. Emits v0.16 bootstrap sidecars as packed sequences with no padding,
...
37. Sets `IndexRoot.payload_block_count` to the sum of distinct payload
    envelope data blocks and `IndexRoot.tar_total_size` to the final
    tar-stream byte offset.
...
23. Rejects non-zero `FileEntry.flags`, unsafe archive paths, and
    ...
    zero compressed/decompressed size, and encrypted objects whose
...
29. Rejects `has_dictionary = 0` with non-zero IndexRoot dictionary
    ...
    offsets, IndexShard `shard_index` mismatches, invalid
    DirectoryHintShardEntry ordering or shard-list ranges, and
...
31. During full-archive `verify`, checks that
    ...
    EnvelopeEntry data-block counts and that `IndexRoot.tar_total_size`
    equals the highest observed payload frame end offset.
...
## Appendix A: All changes from v0.15 → v0.16

| Section | Change |
|---|---|
| §6.1 / §15.9 / §17.2 | Depadded payload length must equal `EnvelopeEntry.plaintext_size` before frame slicing |
| §10 / §29 | BlockRecord bit 0 requirement clarified for all encrypted object data kinds |
| §13.1 | Raw KDF mode explicitly transitions directly to Extension TLVs |
| §14.3 / §17.2 | Metadata objects are depadded before zstd decompression |
| §15.3 / §15.6 / §15.9 | Empty FrameEntry and FileEntry records rejected |
| §15.8 / §15.9 | DirectoryHintEntry shard-list addressing changed from byte offset to u32 element index |
| §15.9 / §17.2 | IndexShard `shard_index` cross-check and zero-file-shard rejection added |
| §15.9 | `tar_total_size` full-archive verification added |
| §27 | Integer ceiling formulas made explicit |
| §28.1 / §29 | v0.16 tests and conformance checklist expanded |

---

*End of v0.16 specification.*
