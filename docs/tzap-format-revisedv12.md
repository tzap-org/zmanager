# tzap Archive Format Specification (v0.12)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.12 / 2026-05-20.6 (draft after footer/streaming/FEC clarification review) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-20 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.11 text, this v0.12 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.11

This revision fixes remaining footer, streaming, FEC, and validation
ambiguities identified after v0.11.

1. **Per-volume ManifestFooter semantics are clarified.** Footer copies
   share archive bootstrap fields but are not byte-identical: each volume
   gets its own authenticated `volume_index`. (§7.1, §11, §19.1)
2. **Previously undocumented fields are now normative.** `FileEntry.flags`
   is reserved and must be zero; `payload_block_count` is defined.
   (§15.2, §15.6, §29)
3. **Encrypted object sizes are unambiguous.** `encrypted_size` now means
   total ciphertext length including AEAD tag after suffix padding, and
   must equal `data_block_count * block_size` in v0.12. (§15.3, §15.4,
   §15.8, §15.9, §18)
4. **Empty archives are explicitly valid, but empty payload envelopes
   are not.** A zero-file archive has no payload envelopes and
   `shard_count = 0`, but still has an IndexRoot, ManifestFooter, and
   trailer. (§15.2, §15.3, §15.9, §19.1)
5. **Reader caps are more concrete.** Per-shard file counts and adjacent
   hash-collision shard scans now have explicit caps. (§13.3, §15.4,
   §15.9)
6. **FEC object layout is fully normative.** Object block ranges are
   contiguous, size fields must fit `u32`, and parity iteration has a
   bounded convergence rule. (§18, §27, §29)
7. **Header/trailer binding is explicit.** Readers must compare
   authenticated trailer/footer identity fields against VolumeHeader
   fields before using bootstrap data. (§17.1)
8. **Path and metadata diagnostics are tightened.** Writers must not emit
   unsafe archive paths, and library readers should return structured
   metadata-fidelity diagnostics. (§16, §29)
9. **Streaming boundaries are explicit.** Multi-volume sequential merging
   is an implementation concern outside the single-volume wire format,
   and dictionary-compressed single-stream archives without an
   already-available sidecar may require unbounded buffering. (§17.3,
   §19.3)
10. **CRC and nonce-generation trust models are documented.** VolumeHeader
   CRC is only a corruption check, and HKDF-Expand nonce generation is a
   deterministic PRF use. (§8, §14.1)
...
- `pad_len = 0` is not valid in v0.12. The extra block in the exact-fit
...
- In wide form, `N ≥ 5` is necessary but not sufficient: readers still
  must enforce `pad_len ≤ N` before subtraction. This rejects malformed
  tiny envelopes whose 4-byte length field would otherwise claim padding
  beyond the decrypted buffer.
...
stream), conforming v0.12 writers MUST use `stripe_width = 1`,
...
treat VolumeHeader identity fields as authenticated until they are
matched against authenticated VolumeTrailer and ManifestFooter fields
after HMAC verification (§17.1).
...
and `header_hmac`. `header_hmac = HMAC-SHA-256(mac_key, all
CryptoHeader bytes before the header_hmac field)`. Readers MUST reject a
CryptoHeader whose length is smaller than the fixed header plus HMAC,
whose TLV list does not terminate before `length - 32`, or whose
...
from `chunk_size` alone. Actual frame sizes are described by FrameEntry.
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.12.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.12
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.12 this
means any flag bit other than bit 0 is invalid.
...
bootstrap metadata; archive content hashes, tar size, envelope count, and
frame count are encrypted inside IndexRoot.
...
Completed v0.12 writers MUST set `is_authoritative = 1` in every closed
...
(`<base>.tzap.bootstrap`). The sidecar may contain:
...
The sidecar header CRC is only an early corruption check. Authority still
comes from the ManifestFooter HMAC plus AEAD verification of IndexRoot
and any copied dictionary object.
...
non-critical by a future version; for v0.12, unknown flag bits are a hard
error.
...
HKDF-SHA-256 Extract followed by Expand exactly as shown. The salt is
the fixed public domain-separation string above; the Argon2id salt
remains the per-archive password-hardening salt in §13.1. Raw-key mode
uses the supplied 32-byte value as `master_key` and still runs the same
HKDF subkey schedule.
...
The dictionary object uses domain `dict` and counter 0. Directory hint
shards use domain `dirhint` and their directory-hint shard index.
...
`envelope_count = 0`, `payload_block_count = 0`, and `tar_total_size = 0`.
No payload envelopes or IndexShard objects are written, but the archive
...
For every encrypted object in v0.12, `encrypted_size` is the total
...
shard, and shard ranges are monotonic.
...
`FileEntry.flags` is reserved in v0.12. Writers MUST set it to zero, and
...
struct DirectoryHintEntry {
    ...
    path_length:            u32,
    _reserved:              u32,
    shard_list_offset:      u64,        // u32 shard IDs, sorted ascending
    shard_count:            u32,
    _reserved2:             u32,
}
...
plaintext of one directory-hint shard object. DirectoryHintEntry records
inside a shard are sorted by `(dir_hash, directory_path)` using bytewise
comparison of normalized UTF-8 directory paths as the collision
...
`shard_list_offset` points into a contiguous u32 shard-ID array; the
`shard_count` IDs for an entry MUST be sorted ascending and unique.
...
path-sorted index and does not by itself define listing order.
...
7. If the input is non-seekable:
     a. If a trusted bootstrap sidecar is supplied, use it for
        ManifestFooter and IndexRoot bootstrap after verifying HMAC/AEAD.
...
11. If has_dictionary = 1: read the dictionary object blocks from the
    ...
    `index_root_key` using domain `dict`, and zstd-decompress (no
...
shard limit for that object class. If an envelope, index shard,
...
`index_root_fec_data_shards` limit and the ReedSolomonGF16 total-shard
limit by increasing files per shard, reducing root-table cardinality
where possible, or otherwise reject with "IndexRoot too large."
...
be zero in v0.12 archives. Readers MUST reject blocks with reserved flag
bits set.
...
11. Build index (compute SHA-256(path) for every FileEntry, sort by hash):
    ...
    b. For each shard: serialize FileEntry records plus the local
     ...
     c. If has_dictionary = 1: zstd-compress the raw dictionary bytes
        ...
        index_root_key using domain `dict`, object-local FEC-encode, and
     ...
     d. If directory hints are required or requested: build one or more
        ...
        AEAD-encrypt with index_shard_key using domain `dirhint`,
...
No part of the v0.12 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.12 mode that round-robins
...
### 20.1 Padding overhead (v0.12 unchanged from v0.11)
...
Same as v0.11. Envelope-level AEAD, object-local FEC encoding, zstd frame
...
- VolumeHeader and BlockRecord CRC32C fields are corruption detectors,
  ...
  object bytes after authenticated trailer/footer checks and AEAD/HMAC
...
implementation claims conformance to this v0.12 draft, incompatible
...
| `index_root_fec_data_shards` | dynamic, minimum 16 | maximum IndexRoot and dictionary-object data blocks; writer MUST raise enough for serialized IndexRoot, but must keep total data+parity shards ≤ 65,535 for ReedSolomonGF16 |
...
MUST reject rather than emit a non-conforming root object.
...
The calculation above chooses each class maximum (`*_parity_shards`) from
that class's maximum data shards (`*_data_shards`). Writers then apply
the same recurrence to each actual object using that object's
`data_block_count`; the resulting `parity_block_count` is stored in
EnvelopeEntry, ShardEntry, or ManifestFooter and MUST NOT exceed the
class maximum.
...
- **Parity recurrence seed**: verify class-max and per-object parity
  ...
  values in §27.
- **HKDF vectors**: fixed passphrase, Argon2id salt/params, and raw-key
  cases with expected `enc_key`, `mac_key`, nonce seeds, and index keys.
  Verify independent implementations derive identical subkeys.
...
- **Large-index root bound**: synthesize an archive plan with enough
  ...
  the IndexRoot FEC limit and verify the writer rejects "IndexRoot too
  large."
- **Directory hint sharding**: build directory hints whose entry table
  ...
  reject any single hint shard exceeding §18 object limits.
...
- **Bootstrap sidecar**: dictionary archive with a bootstrap sidecar;
  verify `TZBS` header CRC, UUID/session binding, ManifestFooter HMAC,
  IndexRoot AEAD, and dictionary-object AEAD are checked; verify
  IndexRoot and dictionary objects decrypt from sidecar BlockRecord
  copies and the dictionary is available before payload frame
  decompression.
...
18. Derives subkeys with the §13.2 HKDF-SHA-256 schedule.
...
22. Sets all reserved BlockRecord flag bits to zero.
...
26. Ensures every encrypted object's `encrypted_size` equals
    `data_block_count * block_size` and fits in `u32`.
...
16. Validates §12.3 sidecar CRC, UUID/session binding, known flag bits,
    ManifestFooter HMAC, IndexRoot AEAD, and dictionary-object AEAD when
    present before trusting bootstrap sidecar bytes.
17. Derives subkeys with the §13.2 HKDF-SHA-256 schedule.
...
19. Rejects BlockRecords with reserved flag bits set.
...
25. Rejects empty payload envelopes and any IndexRoot, IndexShard, or
    DirectoryHintTable object that exceeds its FEC class limits.
...
## Appendix A: All changes from v0.11 → v0.12

| Section | Change |
|---|---|
| §7.1 / §11 / §19.1 | ManifestFooter copies clarified as per-volume authenticated copies, not byte-identical replicas |
| §6.1 / §15.3 / §15.9 / §29 | Empty payload envelopes are explicitly invalid; empty archives remain valid |
| §8 / §12.3 / §18 / §22 | CRC roles clarified as corruption detection only; HMAC/AEAD remains the authority before plaintext release |
| §14.1 | HKDF-Expand nonce generation documented as deterministic PRF use with AEAD-specific output length |
| §15.8 / §18 / §24 | Directory-hint shards and IndexRoot are explicitly bounded by FEC object limits; non-splittable oversized IndexRoot is rejected |
| §17.3 | Multi-volume sequential extraction clarified: tzap defines no multiplexed pipe container or delimiter wrapper |
| §18 | Parity-block corruption handling clarified: repaired ciphertext must still pass AEAD before plaintext release |
| §24 / §27 | Dynamic IndexRoot data-shard ceiling and parity convergence condition documented |
| §28.1 | v0.12 test corpus additions cover per-volume footers, empty-envelope rejection, IndexRoot bounds, parity corruption, and padding-marker boundary fuzzing |

---

*End of v0.12 specification.*